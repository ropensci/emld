...
xsd_recursion <- function(nodeset, grouplist = NULL){
  ...
  if(length(choice) > 0){
    tmp <- xsd_recursion(xml_children(nodeset[[choice]]))
    nodeset <- list_insert(nodeset, tmp, choice)
  }
  sequence <- xml_select(nodeset, "sequence")
  if(length(sequence) > 0){
    tmp <- xsd_recursion(xml_children(nodeset[[sequence]]))
  ...
  if(length(group) > 0){
    ref <- xml_attr(nodeset[[group]], "ref")
...
type_nodes <- map(xsd_files, function(xsd){
  ...
  get_nodes(complex_types, xml, "complexType", grouplist = group_nodes)
})  %>% unlist(FALSE) %>% purrr::compact()
